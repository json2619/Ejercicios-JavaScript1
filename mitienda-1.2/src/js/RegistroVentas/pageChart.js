...
function addSale() {
	...
	try {
		// Validación de los datos recogidos en el formulario
		if (newMonth.value === "") throw {
			name: "MonthError",
			message: "El mes ya está incluido en la gráfica"
		};

		// Insertar la venta en la estructura de datos

		if (monthlySalesMap.has(newMonth.value)) {
			let currentMonthlySales = monthlySalesMap.get(newMonth.value);
			if (monthlySalesMap.has(newProduct.value)) {
			...
			// No tenemos ni el producto ni el mapa
			// Crear un nuevo mapa
			// Insertar la cantidad con la clave de la categoria en el mapa
			// Insertar este mapa en el mapa princiap con clave el mes qre acabamos de recibir
		}
